...
export default function DeckDetails() {
  ...
  useEffect(() => {
    ...
    // Only trigger if previous deck is different from current deck
    if (prevDeckRef.current && prevDeckRef.current !== deck) {
      setUnsavedChanges(true);
    } 

    // Update the ref to the current deck
  ...
  useEffect(() => {
    const handleBeforeUnload = (event: BeforeUnloadEvent) => {
      const shouldWarn = false // set this dynamically if needed
      ...
      <header className="mb-8">
        <div className="flex items-center gap-2 mb-3">
          <div className="max-w-8 max-h-8">
            {user.imageUrl && <img className="rounded-full" src={user.imageUrl}></img>}
          </div>
          <h2>{user?.username}</h2>
        </div>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <FlashcardDialog handleSubmit={handleSubmit} >
              <Button type="button" variant={"default"}><Plus /> Add Card</Button>
            </FlashcardDialog>
